...
func NewVPCEndpointStatisticsStorage(nsxClient *nsx.Client, k8sClient k8sclient.Client, vpcService eas.VPCInfoProvider) *VPCEndpointStatisticsStorage {
...
func (s *VPCEndpointStatisticsStorage) Get(ctx context.Context, namespace, name string) (*easv1alpha1.VPCEndpointStatistics, error) {
	...
	// 1. Get VPCEndpoint CR
	vpcEndpointCR := &vpcv1alpha1.VPCEndpoint{}
	if err := s.k8sClient.Get(ctx, k8sclient.ObjectKey{Namespace: namespace, Name: name}, vpcEndpointCR); err != nil {
		return nil, fmt.Errorf("VPCEndpoint CR %s/%s not found: %w", namespace, name, err)
	}
	...
	// 4. List VPC endpoints and find the one matching the CR name tag
	...
	// 5. Get Statistics from NSX using VPCEndpointStatisticsClient
